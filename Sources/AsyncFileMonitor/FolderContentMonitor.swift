...
public final class FolderContentMonitor: @unchecked Sendable {
	...
	var callback: (@Sendable (FolderContentChangeEvent) -> Void)?
	...
	public func start() {
		guard !hasStarted else {
			assertionFailure("Start must not be called twice. (Ignoring)")
			return
		}
	...
	public func stop() {
		guard hasStarted, let streamRef = streamRef else { return }
