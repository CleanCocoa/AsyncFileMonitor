...
# Cross-platform Swift build for AsyncFileMonitor on Linux
...
# Build the watch CLI tool for Linux
ARG TARGETPLATFORM
RUN --mount=type=cache,target=/workspace/.build,id=build-$TARGETPLATFORM \
    ...
    mkdir -p dist && \
    cp .build/release/watch dist/

# Export the build products to host filesystem:
# $ mkdir -p ./release
# $ docker buildx build --platform linux/arm64,linux/amd64 --target export -o ./release .
FROM scratch AS export
COPY --from=build /workspace/dist/watch /